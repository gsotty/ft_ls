...
/*   Updated: 2017/03/10 12:07:29 by gsotty           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
size_t			len_dir_ls(char *str, t_struc_ls *struc)
{
	...
	while ((result = readdir(dir)) != NULL)
	{
		if (result->d_name[0] != '.' && struc->flag.a_min == 0)
			x++;
		if (result->d_name[0] == '.' && struc->flag.a_min == 1)
			x++;
...
STAT			*creat_stat(char *str, t_struc_ls *struc, size_t len_dir, int x)
{
	...
	struc->buf.name[x] = ft_memalloc(sizeof(char *) * len_dir + 1);
	ft_memset(struc->buf.name[x], 0, sizeof(char *) * len_dir + 1);
	struc->len.namlen = ft_memalloc(sizeof(size_t) * len_dir + 1);
	ft_memset(struc->buf.name[x], 0, sizeof(size_t) * len_dir + 1);
	while ((result = readdir(dir)) != NULL)
	{
		if (result->d_name[0] != '.' && struc->flag.a_min == 0)
		{
			...
			struc->buf.name[x][y] = ft_memalloc(result->d_namlen * sizeof(char));
			ft_memcpy(struc->buf.name[x][y], result->d_name, result->d_namlen);
			struc->buf.name[x][y][result->d_namlen] = '\0';
...
char			**stat_ls_dir(char *str, t_struc_ls *struc, size_t x)
{
	...
	if (S_ISDIR(stat.st_mode) == 1)
	{
		...
		struc->buf.len_dir[x] = len_dir_ls(str, struc);
		len_dir = struc->buf.len_dir[x];
		...
		{
			verif_len(buf, struc);
			while ((y < struc->buf.nbr_files) && (struc->flag.l_min == 1))
			{
				name[y] = creat_l_min(buf[y], struc, struc->buf.name[x][y],
						struc->len.namlen[y]);
				y++;
			}
		}
		...
		{
			while ((y < struc->buf.nbr_files))
			{
				name[y] = ft_strdup(struc->buf.name[x][y]);
				name[y][struc->len.namlen[y]] = '\0';
				y++;
			}
		}
	}
	else if (struc->buf.not_r == 0)
	{
		ft_printf("is not a directory:\n");
		struc->buf.len_dir[x] = 1;
...
char			***readdir_ls(t_struc_ls *struc, char **argv, size_t cont_argc)
{
	...
	x = 0;
	struc->buf.name = ft_memalloc(sizeof(char **) * struc->buf.argc -
			cont_argc);
	ft_memset(struc->buf.name, 0, sizeof(char **) * struc->buf.argc -
			cont_argc);
	struc->buf.len_dir = ft_memalloc(sizeof(unsigned int) *
			struc->buf.argc - cont_argc);
	ft_memset(struc->buf.len_dir, 0, sizeof(unsigned int) *
			struc->buf.argc - cont_argc);
	data = ft_memalloc(sizeof(char **) * struc->buf.argc);
	ft_memset(data, 0, sizeof(char **) * struc->buf.argc);
